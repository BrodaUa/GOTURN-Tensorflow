...
logfile = "test.log"
test_txt = 'Girl2.txt'#"test_set.txt"
...
def data_reader(input_queue):
    ...
    search_tensor = tf.image.resize_images(search_tensor,[HEIGHT,WIDTH], method=tf.image.ResizeMethod.BILINEAR)
    target_tensor = tf.to_float(tf.image.decode_jpeg(target_img, channels = 3))
    target_tensor = tf.image.resize_images(target_tensor,[HEIGHT,WIDTH], method=tf.image.ResizeMethod.BILINEAR)
...
if __name__ == "__main__":
    ...
    try:
        with open('./test_bbox.txt','w') as ftest:
            for i in range(0, int(len(train_box)/BATCH_SIZE)):
                cur_batch = sess.run(batch_queue)
                start_time = time.time()
                [batch_loss, fc4] = sess.run([tracknet.loss, tracknet.fc4],
                    feed_dict={
                        tracknet.image:cur_batch[0],
                ...
                for line in fc4:
                    ftest.write(',,')
                    ftest.write(','.join(map(str,line/10)))
                    ftest.write('\n')
